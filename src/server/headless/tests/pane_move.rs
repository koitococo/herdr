...
async fn public_pane_move_focus_follows_the_moved_pane() {
    ...
    assert_eq!(
        ...
        Some(moved.pane.workspace_id.as_str()),
...
async fn public_pane_move_focus_handles_source_removal_and_unchanged_server_target() {
    ...
    // One move creates a workspace; the other exercises the same-tab no-op.
    for new_workspace in [true, false] {
        ...
        .unwrap();
        assert_eq!(moved.changed, new_workspace);
...
async fn public_pane_move_without_effective_focus_preserves_client_views() {
    for case in ["no-focus", "same-tab", "zoomed", "invalid"] {
        ...
        let destination = if case == "no-focus" {
            PaneMoveDestination::NewWorkspace {
                label: None,
                tab_label: None,
            }
            ...
            let tab_id = match case {
                "same-tab" | "zoomed" => server.app.public_tab_id(0, 0).unwrap(),
                "invalid" => "missing-tab".into(),
                _ => unreachable!("no-focus handled above"),
            };
            PaneMoveDestination::Tab {
                tab_id,
                target_pane_id: None,
        ...
        if case == "invalid" {
            ...
            assert_eq!(moved.changed, case == "no-focus");
            if case == "no-focus" {
                let location = server.clients[&9]
                    .shell_location
                    .as_ref()
                    .expect("client shell location");
                let previous_location = location_before.as_ref().expect("previous location");
                assert_eq!(
                    location.focused_workspace_id, previous_location.focused_workspace_id,
                    "{case}"
                );
                assert_eq!(
                    location.active_tab_ids.len(),
                    previous_location.active_tab_ids.len() + 1,
                    "{case}"
                );
            } else {
                assert_eq!(server.clients[&9].shell_location, location_before, "{case}");
