...
/// Collapsed sidebar dedicates its full height to workspaces.
pub(crate) fn collapsed_workspace_list_rect(area: Rect) -> Rect {
    Rect::new(area.x, area.y, area.width.saturating_sub(1), area.height)
}
...
/// Collapsed sidebar workspace glance.
pub(super) fn render_sidebar_collapsed(app: &AppState, frame: &mut Frame, area: Rect) {
    ...
    let ws_area = collapsed_workspace_list_rect(area);
    if ws_area == Rect::default() {
...
"##,
    ...
    #[test]
    fn agent_entries_keep_tab_labels_for_legacy_multi_tab_workspaces() {
        let mut app = crate::app::state::AppState::test_new();
        ...
        // Mobile still presents legacy restored tabs, although desktop does
        // not create a second tab.
    ...
    #[test]
    fn priority_agent_entry_sort_uses_attention_then_space_order() {
        let mut app = crate::app::state::AppState::test_new();
        app.workspaces = vec![
    ...
    #[cfg(unix)]
    #[tokio::test]
    async fn agent_entries_use_live_root_runtime_cwd_for_workspace_label() {
        let unique = format!(
    ...
    #[test]
    fn agent_entries_prefer_agent_names_for_mobile_identity() {
        let mut app = crate::app::state::AppState::test_new();
    ...
    #[test]
    fn sidebar_spaces_use_full_height_without_agent_detail_or_divider() {
        let area = Rect::new(0, 0, 20, 5);

        assert_eq!(workspace_list_rect(area), Rect::new(0, 0, 19, 5));
        assert_eq!(collapsed_workspace_list_rect(area), Rect::new(0, 0, 19, 5));
    }
