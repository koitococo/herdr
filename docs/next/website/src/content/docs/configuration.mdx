...
Set the working directory policy for new panes and workspaces, including each workspace's initial tab:
...
Grouped worktrees still behave like normal Herdr workspaces: they can be focused, renamed, closed, and contain panes with retained tab identity. The parent row is the original workspace. Closing the parent row closes the whole Herdr group, but it does not delete checkout folders or branches.
...
Desktop tab-action bindings remain parsed for compatibility but cannot create, switch, move, close, or rename tabs. Use workspace bindings for separate layouts.
...
The desktop sidebar is the main Herdr dashboard and shows Spaces across its full height. Desktop no longer renders a tab row or an Agent panel. The existing `ui.hide_tab_bar_when_single_tab`, `ui.tab_bar_*`, `ui.agent_panel_sort`, and `ui.sidebar.agents.*` settings remain accepted for configuration compatibility, but they have no effect in the desktop UI. This change does not alter the mobile UI.
...
The legacy tab-row settings below remain accepted for existing configurations, but the Spaces-only desktop UI does not render a tab row and these settings have no desktop effect:

Set `tab_bar_position = "bottom"` under `[ui]` in configurations that still use a tab row to place it below the terminal panes. Prefix, Navigate, Copy, and Resize mode bars temporarily replace the bottom tab row while active. The default is `"top"`.

Configure an ordered tmux-style status area at the right edge of a tab row:
...
Separators appear only between visible entries. Set `tab_bar_right_separator = ""` for direct concatenation. On a narrow tab row, the complete status area yields to the tabs and their controls. These settings are retained for compatibility and are not rendered by the Spaces-only desktop surface.
...
### Space sidebar row layouts

The expanded desktop sidebar renders Spaces only. Each inner array in `rows` is one line:
...
The removed desktop Agent panel is not configured by this release. Its retained `[ui.sidebar.agents]` settings have no desktop UI effect.
...
[ui.sidebar.spaces]
rows = [
  ["state_icon", { token = "workspace", bold = false }],
  [{ token = "$summary", fg = "#89b4fa", bold = true, dim = false }],
]
```

`fg` accepts strict `#RGB` or `#RRGGBB`. `bold` and `dim` accept booleans. Omitted fields preserve the token's contextual style; explicit `false` removes that modifier. Styling applies to one occurrence, so the same token may look different in another row. A foreground override replaces all semantic foregrounds inside that occurrence: for example, styled `git_status` ahead and behind counts use one color instead of their default green and red. Token styles never change separators or row backgrounds.

`row_gap` controls blank terminal rows between Space groups. It defaults to `0`, which packs entries together; set it to `1` to restore the previous spacing. It does not add spacing between the content lines declared in `rows`. Consecutive indented worktree children remain packed as one Space group.

Custom `$name` tokens are dynamic values, not literal text. Add the token to a Space layout, then report its value from a script or plugin:

```toml
[ui.sidebar.spaces]
rows = [
  ["state_icon", "workspace", "$model"],
  ["$summary"],
  ["branch", "git_status"],
]
```
...
Unreported custom tokens disappear. Metadata reporters provide values only; styling stays in the local sidebar configuration. See [CLI reference: report metadata](/docs/cli-reference/#workspaces) for limits, clearing, sequencing, and expiry.

Sidebar Space row settings affect only the expanded desktop sidebar. Collapsed and mobile views keep their compact layouts.
...
Cap how often Herdr redraws the local TUI and headless server UI:
...
Valid range is 1–60 Hz. The default is 60. Lower values reduce redraw CPU and terminal write volume when the UI is dirty. This is a process-local render gate, not the client-side `HERDR_CLIENT_REFRESH_RATE_HZ` semantic-frame pacing env var. Invalid values are rejected on load or reload, and the previous value is kept.
...
| `HERDR_CLIENT_REFRESH_RATE_HZ` | Limit one thin client's semantic-frame presentation to an integer 1–60 Hz. Read at client startup; does not change server rendering or ANSI/direct-terminal clients. |
