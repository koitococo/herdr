...
- list and inspect tab identities
...
Tab records, `tab_id` fields, and `HERDR_TAB_ID` remain part of the protocol and runtime. `tab.list` and `tab.get` remain available, including for restored legacy multi-tab sessions. The server rejects `tab.create`, `tab.move`, `tab.close`, and `tab.rename` with `multi_tab_unsupported`; `tab.focus` succeeds only for the already active tab and never switches tabs, while a focus request for another tab returns the same error. Every rejected tab mutation uses the message `multi-tab mutations are not supported; use workspaces instead`. This is not a protocol or snapshot migration.

`pane.move` rejects `new_tab` and cross-tab destinations with the same error; same-tab moves and `new_workspace` remain available. `layout.apply` only accepts replacement of the unique tab when its `tab_id` is supplied; paths that would create a tab are rejected. Plugin pane placement `tab` is also rejected.
...
records. Full repo worktree discovery remains `worktree.list`. Legacy multi-tab
snapshots continue to restore and enumerate; clients must not rely on desktop
management or switching of their non-active tabs.
...
`layout.apply` replaces the unique tab from a declarative tree only when its
`tab_id` is provided. Requests that would create another tab are rejected with
`multi_tab_unsupported`. A replacement restores structure, labels, cwd, env, and
optional argv commands; it does not preserve live PTYs, scrollback, or running
processes.
...
`pane.move` retains same-tab layout changes and moves to a new workspace:

```json
{"id":"req_move_same_tab","method":"pane.move","params":{"pane_id":"w1:p2","destination":{"type":"tab","tab_id":"w1:t1","target_pane_id":"w1:p3","split":"right","ratio":0.5},"focus":true}}
...
Same-tab moves require `split: "right" | "down"`. `target_pane_id` is optional
and defaults to the active tab's focused pane. Moving to the source tab returns
`changed: false` with `reason: "same_tab"`; moves involving a zoomed tab return
`changed: false` with `reason: "zoomed_tab"`. `new_tab` and any cross-tab
destination are rejected with `multi_tab_unsupported`.
...
`zoomed`. Placement `tab` is rejected with `multi_tab_unsupported`. Overlay and
popup placements use the active tiled pane as launch context. Popup terminals
are session-modal and do not change the tab layout; optional `width` and
`height` fields set their outer size as terminal cells or percentages such as
`"80%"`. Omitted dimensions default to half the terminal size, with too-small
values clamped to the popup minimum. A popup has no pane ID, remains outside all
`pane.*` and agent APIs, emits no pane lifecycle events, leaves plugin focus
context on the underlying tiled pane, and does not export `HERDR_PANE_ID` to its
process. Popup launch returns `ok`; `popup.close` closes the active popup and
returns `popup_not_open` when none exists. Split, zoomed, and overlay panes
target an existing pane and behave like normal Herdr panes; `plugin.pane.focus`
and `plugin.pane.close` continue to operate on those panes.
...
Tab-topology requests fail like this:

```json
{
  "id": "req_tab",
  "error": {
    "code": "multi_tab_unsupported",
    "message": "multi-tab mutations are not supported; use workspaces instead"
  }
}
```
