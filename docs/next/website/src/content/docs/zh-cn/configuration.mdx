...
侧边栏是 Herdr 的主仪表盘。桌面端会在侧边栏的整个高度显示 Spaces，不再显示标签栏或 Agent 面板。尺寸、折叠模式、鼠标行为、窗格边框和其他显示设置可在[配置参考](/docs/config-reference/)中搜索 `ui.` 查看。现有的 `ui.hide_tab_bar_when_single_tab`、`ui.tab_bar_*`、`ui.agent_panel_sort` 和 `ui.sidebar.agents.*` 设置仍会出于配置兼容性而接受，但不会影响桌面 UI。此变更不会改变移动端 UI。
...
以下旧版标签栏设置仍会为已有配置而接受，但 Spaces-only 桌面 UI 不会显示标签栏，这些设置也不会影响桌面端。

对于仍使用标签栏的兼容表面，在 `[ui]` 下设置 `tab_bar_position = "bottom"` 可将其放到终端窗格下方。Prefix、Navigate、Copy 和 Resize 模式栏会在显示期间临时替换底部标签栏。默认值为 `"top"`。

对于仍使用标签栏的兼容表面，可在标签栏右侧配置一个类似 tmux 的有序状态区：
...
状态区默认为空。在兼容的标签栏表面添加 `zoom` 后，活动标签页处于缩放状态时会显示固定的 `ZOOM` 标记；它与现有的逐标签页 `Z` 标记相互独立。`hostname`、`datetime` 和 `command` 在 Herdr 服务器上解析，因此使用 `herdr --remote` 时会显示远程机器的值。日期时间条目使用 `strftime` 格式。由于该值是服务器本地的墙上时钟时间，需要 UTC 偏移量或 Unix 时间戳的 `%z`、`%s` 等指令会被拒绝。
...
分隔符只会出现在可见条目之间。设置 `tab_bar_right_separator = ""` 可直接连接条目。兼容的标签栏较窄时，整个状态区会让位给标签页及其控件。这些设置会为兼容性保留，但不会在 Spaces-only 桌面表面渲染。
...
展开的桌面侧边栏只显示 Spaces。`rows` 中的每个内层数组都会渲染为一行：
...
此版本不会在桌面端显示 Agent 面板。保留的 `[ui.sidebar.agents]` 设置仍会为配置兼容性而接受，但不会影响桌面 UI。Agent 相关的配置 schema 和移动端 UI 均未改变。

Agent 行设置支持以下内置 token：
...
`row_gap` 分别控制 Agent 行和 Space 行设置中条目之间的空白终端行数。默认值为 `0`，会紧密排列条目；设为 `1` 可恢复之前的间距。它不会在 `rows` 声明的内容行之间添加间距。连续缩进的 worktree 子项仍会作为一个 Space 组紧密排列。

在 `rows_by_agent` 下为已知智能体覆盖 Agent 行的设置布局：
...
有效范围为 1–60 Hz，默认值为 60。降低该值可在界面 dirty 时减少重绘 CPU 与终端写入量。这是进程内的渲染门控，不是客户端 semantic frame 节流环境变量 `HERDR_CLIENT_REFRESH_RATE_HZ`。无效值会在加载或重载时被拒绝，并保留先前的值。
