...
Tabs remain addressable protocol objects inside a workspace. `tab list` and
`tab get` remain available so automation can inspect tab identity, including
restored legacy multi-tab sessions. `tab create`, `tab rename`, and `tab close`
fail with `multi_tab_unsupported`, with the message `multi-tab mutations are not
supported; use workspaces instead`. `tab focus` succeeds only for the already
active tab and never switches tabs; a request to focus another tab returns that
same error. Use workspaces, not tabs, for separate
layouts. Workspace creation still creates its initial tab and root pane, and its
JSON response exposes `.result.workspace.workspace_id`, `.result.tab.tab_id`,
and `.result.root_pane.pane_id`. This release does not migrate the protocol or
snapshots.
...
`pane move --new-tab` and moves to a different tab fail with
`multi_tab_unsupported`. Same-tab moves and `pane move --new-workspace` remain
available.
...
`--token` patches one named display value; `--clear-token` removes one. Unmentioned tokens remain unchanged. Pane tokens remain available through pane and agent API responses and the retained Agent-sidebar configuration, but do not render in the desktop UI; workspace tokens are available to Space rows. TTL applies independently to the token keys updated by that call.
...
Agent runtime and CLI commands remain available; removing the desktop Agent
sidebar does not remove detection, reporting, orchestration, or direct attach.
...
zoomed pane, or a session-modal `popup` that does not change the tab layout.
`--placement tab` fails with `multi_tab_unsupported`. `--width` and `--height`
set the outer popup dimensions in terminal cells or percentages such as `80%`;
omitted dimensions default to half the terminal size, and values smaller than
the popup minimum are clamped. A popup is not a Herdr pane, does not export
`HERDR_PANE_ID`, and does not participate in pane or agent APIs. Native
non-terminal plugin panes are outside plugin v1.

`--env KEY=VALUE` can be repeated on process-launching commands. It applies to
the newly launched process only. Herdr-managed variables such as
`HERDR_SOCKET_PATH`, `HERDR_BIN_PATH`, `HERDR_ENV`, `HERDR_WORKSPACE_ID`,
`HERDR_TAB_ID`, `HERDR_PANE_ID`, `HERDR_PLUGIN_ID`, `HERDR_PLUGIN_ROOT`,
`HERDR_PLUGIN_CONFIG_DIR`, `HERDR_PLUGIN_STATE_DIR`,
`HERDR_PLUGIN_ENTRYPOINT_ID`, and `HERDR_PLUGIN_CONTEXT_JSON` stay
...
| `HERDR_TAB_ID` | Public tab id for the running pane process. It remains available for runtime and protocol compatibility. |
