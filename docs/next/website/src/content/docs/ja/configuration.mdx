...
サイドバーは Herdr のメインダッシュボードです。デスクトップではサイドバー全体に Spaces が表示され、タブ行や Agent パネルは表示されません。サイズ、折りたたみモード、マウスの挙動、ペインの境界線、その他の表示設定については、[設定リファレンス](/docs/config-reference/)で `ui.` を検索してください。既存の `ui.hide_tab_bar_when_single_tab`、`ui.tab_bar_*`、`ui.agent_panel_sort`、`ui.sidebar.agents.*` 設定は構成互換性のため引き続き受け付けられますが、デスクトップ UI には影響しません。この変更はモバイル UI を変更しません。
...
以下のレガシーなタブ行設定は既存の構成のため引き続き受け付けられますが、Spaces-only のデスクトップ UI はタブ行を表示せず、これらの設定はデスクトップには影響しません。

`[ui]` の `tab_bar_position = "bottom"` は、タブ行を引き続き使う互換性のあるサーフェスで、ターミナルペインの下に配置します。Prefix、Navigate、Copy、Resize の各モードバーは、表示中だけ下部のタブ行を置き換えます。デフォルトは `"top"` です。

タブ行を引き続き使う互換性のあるサーフェスでは、タブ行の右端に tmux のような順序付きステータス領域を設定できます:
...
ステータス領域はデフォルトでは空です。互換性のあるタブ行サーフェスで `zoom` を追加すると、アクティブなタブがズーム中のとき固定の `ZOOM` ピルが表示されます。各タブの既存の `Z` マーカーとは独立しています。`hostname`、`datetime`、`command` は Herdr サーバー上で解決されるため、`herdr --remote` ではリモートマシンの値が表示されます。日時エントリは `strftime` 形式を使います。`%z` や `%s` のように UTC オフセットまたは Unix タイムスタンプを必要とする指定子は、値がサーバーローカルの壁時計時刻であるため拒否されます。
...
区切り文字は、表示されているエントリの間にだけ挿入されます。直接連結するには `tab_bar_right_separator = ""` を設定します。互換性のあるタブ行が狭い場合、ステータス領域全体がタブとそのコントロールに場所を譲ります。これらの設定は互換性のために保持されますが、Spaces-only のデスクトップサーフェスでは描画されません。
...
展開されたデスクトップサイドバーには Spaces のみが表示されます。`rows` の各内側の配列が 1 行として描画されます:
...
デスクトップの Agent パネルはこのリリースでは表示されません。保持されている `[ui.sidebar.agents]` 設定は構成互換性のため引き続き受け付けられますが、デスクトップ UI には影響しません。Agent 関連の設定スキーマとモバイル UI は変更されていません。

Agent 行の設定では、次の組み込みトークンを使えます:
...
`row_gap` は、Agent 行と Space 行の設定ごとに、エントリ間の空白行数を指定します。デフォルトは `0` で、エントリを詰めて表示します。以前の間隔に戻すには `1` に設定します。`rows` で定義したコンテンツ行の間隔には影響しません。連続するインデントされた worktree の子は、1 つの Space グループとして詰めて表示されます。

既知のエージェントについて Agent 行の設定レイアウトを上書きするには、`rows_by_agent` を使います:
...
ローカル TUI とヘッドレスサーバー UI の再描画頻度を制限します:
...
有効範囲は 1–60 Hz で、デフォルトは 60 です。値を下げると UI が dirty なときの再描画 CPU とターミナル書き込み量を抑えられます。これはプロセス内の描画ゲートであり、クライアント側の semantic frame pacing 用環境変数 `HERDR_CLIENT_REFRESH_RATE_HZ` とは別です。無効な値は読み込み時・リロード時に拒否され、以前の値が保持されます。
