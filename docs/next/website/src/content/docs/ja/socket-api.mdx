...
`layout.apply` は `tab_id` を使って既存の一意なタブを置き換えます。`tab_id` を省略したリクエスト、または別のタブを作成するリクエストは `multi_tab_unsupported` で拒否されます。これは構造、ラベル、cwd、env、任意の argv コマンドを復元しますが、ライブの PTY、スクロールバック、実行中プロセスは保存しません。
