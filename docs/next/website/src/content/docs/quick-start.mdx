...
When a session has no workspaces, Herdr opens one automatically. A workspace is a project-level container for panes and agents. Give each active project its own workspace to keep agent state readable.
The desktop uses Spaces (workspaces) for separate layouts. Tab identity remains available through compatibility APIs, and legacy mobile views can navigate existing tabs; creating or mutating tabs is not supported.

## Use the mouse

Herdr is mouse-native, so start by clicking panes and workspaces to focus them. Drag split borders to resize. Right-click for context menus, including splitting panes. Double-click a workspace card's content to open the existing rename dialog; click a worktree group's arrow to expand or collapse it. Drag-select text to copy it to your clipboard; double-click a token to select it, or hold the second press and drag to extend the selection by whole words. Both gestures copy when you release the mouse. Copying does not require Ctrl+C.
...
Or `codex`, `pi`, `opencode`, or any other [supported agent](/docs/agents/). Herdr detects it automatically. Agent state remains available across every workspace, showing whether each agent is `working`, `blocked`, `done`, or `idle`, so you always know which project needs you.
