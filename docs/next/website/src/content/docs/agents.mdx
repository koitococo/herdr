...
Herdr is built for running more than one coding agent at a time. Each agent stays in a real terminal pane with its shell, logs, prompts, and running processes intact. Herdr tracks which panes contain agents and rolls their state up to workspaces, so you can see which project needs attention. The desktop shows those workspaces in its Spaces sidebar; it no longer has a separate Agent sidebar. Agent detection, reporting, and CLI automation remain available.
...
The desktop Spaces sidebar rolls state upward.

A blocked agent makes its pane and workspace look blocked. A working agent makes the workspace look active. A done agent stays visible until you view it.

This is the main Herdr workflow: start several agents, let them work in parallel, and use the Spaces sidebar to see which project needs a decision, which one is still running, and which one is ready to review. The mobile UI is unchanged by this release.
...
`state` controls waits, notifications, and rollups. The `summary` token is display-only. It remains available to reports and API consumers, but the removed desktop Agent sidebar does not render Agent rows. Agent runtime state and CLI behavior are unchanged.
